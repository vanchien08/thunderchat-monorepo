...
export class UserService {
  ...
  async resetPasswordWithToken(resetToken: string, newPassword: string) {
    ...
    const emailCandidates = await this.PrismaService.user.findMany({ select: { email: true } })
    ...
    const user = await this.PrismaService.user.findUnique({ where: { email: matchedEmail } })
    if (!user) {
    ...
    await this.PrismaService.user.update({ where: { id: user.id }, data: { password: hashed } })
    await this.storeDel(this.tokenKey(matchedEmail))
  }
